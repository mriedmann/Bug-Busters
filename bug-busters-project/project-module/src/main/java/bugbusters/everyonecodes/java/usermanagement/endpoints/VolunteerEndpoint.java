...
import bugbusters.everyonecodes.java.security.ForbiddenException;
...
public class VolunteerEndpoint {
    ...
    VolunteerPrivateDTO editVolunteerData(@Valid @RequestBody VolunteerPrivateDTO edits, Authentication authentication) {
        if(!edits.getUsername().equals(authentication.getName())) throw new ForbiddenException("Not allowed to edit other users data");
        return volunteerService.editVolunteerData(edits).orElse(null);
    }
    ...
    UserPublicDTO viewClientPublicData(@PathVariable String username) {
